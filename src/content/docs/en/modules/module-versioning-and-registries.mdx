...
A module's **`version`** constraint matters for the same reason a provider version constraint does: without one, a routine `terraform init` or `init -upgrade` can silently resolve a newer Registry or Git-tagged module release with breaking changes nobody reviewed. The **pessimistic constraint operator**, `~> 9.0`, allows any `9.x` release but blocks the `10.0.0` major bump, making upgrades a deliberate decision instead of an accident. Modules specific to one organization belong behind a **private Git source** or **private registry** instead of the public one, and a module you do publish publicly must live in a repository named `terraform-<PROVIDER>-<NAME>` for the Registry to pick it up correctly.
</Callout>
...
export const quizModuleVersioning = [
  ...
  {
    q: 'What does the constraint version = ~> 9.0 allow and what does it block',
    options: [
      'It allows any 9.x release and blocks 10.0.0',
      'It allows only exactly version 9.0 and blocks every other release',
      'It allows every future major version and blocks only 9.0 itself',

...
**`output`** block เปิดเผยค่าหลัง `apply` — ดูได้ด้วย `terraform output`, `terraform output <name>`, หรืออ่านแบบ machine-readable ผ่าน `terraform output -json` และซ่อนจาก CLI display ได้ด้วย `sensitive = true` (ค่ายังลง state อยู่ดี) **`locals`** block คือ named value แบบที่สามใน Terraform ต่างจาก `variable` ตรงที่ caller ตั้งค่าเองไม่ได้ และต่างจาก `output` ตรงที่ไม่ได้เปิดเผยออกไปข้างนอก แค่คำนวณ expression ครั้งเดียวเพื่อไม่ต้องเขียนซ้ำในหลาย resource output สำคัญเกินขอบเขต root configuration เดียว เพราะเป็นกลไกที่ทำให้ module หนึ่งหรือ Terragrunt unit ที่ apply แยกกันหนึ่งส่งค่าเข้าไปอีกอันหนึ่งได้
</Callout>

...
`sensitive = true` แค่ redact ค่าจาก **CLI output และ log** เท่านั้น — ค่านั้นยังถูกเขียนลง `terraform.tfstate` แบบ **plaintext** ดังนั้นจึงไม่ใช่ตัวแทนของการป้องกัน state file เอง การป้องกันจริงคือพึ่ง GCS server-side encryption แบบ at rest และจำกัด IAM role อย่าง `roles/storage.objectViewer` ให้แคบ ให้แค่ principal ที่ต้องอ่าน state bucket จริง ๆ สำหรับ credential ที่อยู่ยาว ให้เลือกดึงค่าจาก service อย่าง Secret Manager ผ่าน `data` source ตอน apply แทนการเก็บเป็น Terraform variable เพราะ source of truth ของ secret จะไปอยู่ในระบบที่ออกแบบมาเพื่อ access control และ rotation แทนที่จะอยู่ใน state
</Callout>
...
export const quizSensitive = [
  {
    ...
    options: [
      'เข้ารหัสค่าใน terraform.tfstate ทำให้อ่านไม่ได้แม้จะเข้าถึงไฟล์ได้',
      'redact ค่าจาก CLI output อย่าง plan และ apply log แต่ค่านั้นยังถูกเขียนลง state แบบ plaintext',
      'ทำให้ค่าไม่ถูกเก็บลง state เลย',
      'หมุนเวียนเปลี่ยนค่าให้อัตโนมัติตามตารางเวลา',
    ],
    answer: 1,
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      'เพราะ data source รันก่อนที่จะ config GCS backend เสียอีก',
    ],
    answer: 1,
  },
  {
    ...
    options: [
      ...
      'ยังไม่มีขั้นตอน review ตามธรรมชาติก่อนจะ apply การเปลี่ยนแปลงกับ shared state และการที่ทุกคน apply จาก laptop ตัวเองก็ยังขยายสเกลได้ไม่ดี',

...
**`source`** argument บน `module` block ชี้ว่าโค้ดของ module อยู่ที่ไหน — local path สำหรับ module ที่ยังผูกกับ repository เดียว, public **Terraform Registry** สำหรับ module ของชุมชนที่ดูแลดีและ resolve ผ่าน `namespace/name/provider`, หรือ **Git URL** (pin ได้ด้วย `?ref=`) สำหรับ module ส่วนตัวหรือภายในองค์กร infrastructure จริงถูกประกอบขึ้นจาก module เล็ก ๆ ที่ focus ชัดเจนหลายตัวต่อกัน ด้วยการส่ง `output` ของ module หนึ่งเป็น input `variable` ของอีก module และ Terraform resolve dependency ระหว่างกันให้อัตโนมัติ การเลือกใช้ module จาก Registry ที่คนใช้กันเยอะอย่าง `terraform-google-modules/network/google` มักเป็นจุดเริ่มต้นที่ดีกว่าการเขียนเองตั้งแต่ศูนย์ ตราบใดที่คุณยังอ่านสิ่งที่ module สร้างก่อนจะเชื่อใจใน production
</Callout>
...
export const quizModuleComposition = [
  ...
  },
  {
    ...
    options: [
      'hardcode ชื่อ network ไว้ใน main.tf ของตัวเอง',
      ...
      'อ่าน network ID จาก local state file ที่ vpc module เขียนไว้',
    ],
    answer: 1,
  },
  {
  ...
  },
  {
    ...
    options: [
      ...
      'เมื่อ module นั้นแก้ปัญหาแบบที่พบบ่อยและเข้าใจกันดีอยู่แล้ว และคุณยังอ่านสิ่งที่ module สร้างก่อนจะเชื่อใจใน production',

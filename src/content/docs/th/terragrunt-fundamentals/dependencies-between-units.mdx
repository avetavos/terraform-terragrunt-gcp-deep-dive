...
`dependency "vpc" { config_path = "../vpc" }` block คือวิธีที่ unit หนึ่งอ่าน output ของอีก unit เป็น `dependency.vpc.outputs.<name>` ข้าม state boundary ที่ module composition ธรรมดาของ Terraform ข้ามไม่ได้ แล้วส่งค่านั้นตรงเข้า `inputs` ของ unit ที่อ่านค่า `mock_outputs` ที่ scope ด้วย `mock_outputs_allowed_terraform_commands` ทำให้ `plan` บน unit ที่พึ่งพาสำเร็จได้ก่อนที่จะ apply unit ที่พึ่งพาจริง ส่วน `apply` จริงยังต้องการ output จริงอยู่ดี เพราะทุก `dependency` block เรียกชื่อ unit ที่พึ่งพาตรง ๆ Terragrunt จึงคำนวณลำดับ apply ที่ถูกต้องทั่วทั้ง tree ของ unit ได้เอง โดยไม่มีใครต้อง hard-code ลำดับนั้น
</Callout>
...
export const quizDependency = [
  ...
  },
  {
    ...
    options: [
      ...
      'ให้ค่า placeholder เพื่อให้ command อย่าง plan สำเร็จได้ก่อนที่จะ apply unit ที่พึ่งพาจริง',
  ...
  },
  {
    ...
    options: [
      ...
      'สร้างลำดับจาก dependency block ที่แต่ละ unit ประกาศไว้ apply unit ที่ถูกพึ่งพาก่อน unit ที่อ่าน output นั้น',
